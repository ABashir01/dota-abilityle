...
#     try:
#         with open("backend/django/Dota/abilitydata/table-population/connection_details.json", "r") as f:
#             data = json.load(f)
#             return data
...
def download_images(url, folder, name) -> str:
    ...
    file_name = os.path.join(folder, name.replace(" ", "-") + ".jpg")
...
def get_ability_values(url) -> dict:
    ...
    for element in desc_div:
        if isinstance(element, str):  # Check if the element is a string
            ...
            # Use .text to extract the text content of the element, even if it's a <span>
    ...
    for i in range(4,len(desc_div)): #Start on the 4th index
        ...
        if element.name == 'span' and element.get('style') == 'color: #00ffff':
            continue
        ...
        if str(element) == '<br/>':
            ...
            val_string += element.text.strip().replace(":", ": ")
...
def populate_table(table_elements, cur):
    ...
    for i in range(len(table_elements)):
        ...
        try:
            query = """INSERT INTO dota_2_abilities (id, name, hero,  
                    description, mana_cost, cooldown, ability_list) 
                    VALUES (%s, %s, %s, %s, %s, %s, %s);"""
            data = (i, ability_name, hero_name,
